...
class DynamicValidationDetector:
    ...
    def generate_comprehensive_dq_rules(self, df: pd.DataFrame, profiles: Dict, 
                                       client_rules: List[Dict] = None) -> pd.DataFrame:
        """
        Generate comprehensive DQ rules combining CLIENT and AI rules into ONE TABLE
        
        Args:
            df: DataFrame to analyze
            profiles: Column profiles
            client_rules: Pre-extracted client rules from Excel metadata
            
        Returns:
            Single DataFrame with all rules (client + AI) combined
        """
        # Start with client rules if provided
        all_rules = []
        
        if client_rules:
            all_rules.extend(client_rules)
            st.info(f"✅ Loaded {len(client_rules)} client-provided rules from Excel metadata")
        
        # Generate AI rules
        ai_rules = self.detect_all_validations(df, profiles)
        all_rules.extend(ai_rules)
        
        if ai_rules:
            st.info(f"✅ Generated {len(ai_rules)} AI-powered rules")
        
        # Validate data against ALL rules (both client and AI)
...
def _generate_excel_report():
    ...
    try:
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            if state.unified_rules_generated and state.unified_validation_rules is not None and not state.unified_validation_rules.empty:
                unified_df = state.unified_validation_rules.copy()
                col_order = ['S.No', 'Business Field', 'Dimension', 'Data Quality Rule', 'Issues Found', 'Issues Found Example', 'Source']
                col_order = [c for c in col_order if c in unified_df.columns]
                unified_df = unified_df[col_order]
                
                unified_df.to_excel(writer, sheet_name='All DQ Rules', index=False)
                
                dq_worksheet = writer.sheets['All DQ Rules']
                _apply_dq_rules_styling(workbook, dq_worksheet, unified_df)
            else:
                pd.DataFrame({'Message': ['No validation rules generated. Run complete analysis first.']}).to_excel(
                    writer, sheet_name='All DQ Rules', index=False)
...
def _generate_json_report():
    ...
    try:
        ...
        report = {
            ...
            'all_dq_rules': state.unified_validation_rules.to_dict('records') if state.unified_rules_generated and state.unified_validation_rules is not None else [],
            'match_rules': generate_match_rules(df, profiles)
        }
